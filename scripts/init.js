window.addEventListener("DOMContentLoaded", () => {
  ...
  const app = new App();
  app.fetchSubjects();
  const utilities = new Utitlities();
...
class Constants {
  ...
  static constantsInit() {
    if (this.#constants) return this.#constants;
    this.#constants = {
      API_URL: "~/api/quizzes.json",
    };
    return this.#constants;
  }

  static get constants() {
    return this.constantsInit();
  }
}

class ApiActions {
  constructor() {
    const constans = Constants.constants;
    this.API_URL = constans.API_URL;
  }
  ...
  async getSubjects() {
    try {
      ...
      if (!response.ok) {
        throw new Error("HTTP await response.json();error " + response.status);
      }
  ...
  async getQuizzes(subjectId) {
    try {
      ...
      const quizzes = response.find((subject) => subject.id === subjectId);
...
class App {
  ...
  async getSubjects() {
    try {
      const response = await fetch("../api/quizzes.json");
      if (!response.ok) {
        throw new Error("HTTP await response.json();error " + response.status);
      }
      return await response.json();
  ...
  async fetchQuizzes(subjectId) {
    ...
    const quizzes = await this.getQuizzes(subjectId);
